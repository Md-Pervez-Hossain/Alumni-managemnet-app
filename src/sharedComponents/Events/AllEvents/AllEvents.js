  ...
  return (
    <div className="max-w-5xl mx-auto sm:flex gap-3">
      ...
      <div className="w-full border-1 bg-gray-300 sm:hidden block mt-10 p-3">
        ...
        >
          {EventCategory.map((item, i) => (
            <option key={i} className="text-xl font-semibold" value={item.eventCategory}>
              {item.label}
            </option>
          ))}
        </select>
        ...
        <div>
          ...
          <div>
            {eventItems.slice(previous, next).map((item) => (
              <div key={item._id} className="md:flex gap-3 justify-between items-center">
                {/* image */}
                <div className="md:w-[280px] w-full rounded-tl-xl rounded-br-xl">
                  <img
                    className="md:w-[280px] w-full rounded-tl-xl rounded-br-xl "
                    src={item.image}
                    alt={item.image}
                  />
                </div>
